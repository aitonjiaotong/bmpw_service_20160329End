...
public interface ZcCarRespository extends JpaRepository<ZcCar,Integer>{
    ...
    @Query("from ZcCar as c where c.model=(:model) and c.type=(:ctype) and c.box=(:box) and c.pailiang=(:pailiang) and c.seat=(:seat) and c.status=0")
    List<ZcCar>find(@Param("model")String model,@Param("ctype")String ctype,@Param("box")String box,@Param("pailiang")String pailiang,@Param("seat")Integer seat);
    ...
    @Query("select c  from ZcCar as c where status=0")
    List<ZcCar> findCanUseCars(Pageable pageable);
}
